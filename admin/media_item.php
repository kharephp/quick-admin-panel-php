...
if(isset($_POST['addgallery']))
{
	...
	$pdo=getPDOObject();
	$posted_data=$_POST;
	$sql=$pdo->query("SELECT * FROM `gallery` where  name LIKE '$name' or seo_url LIKE '$seo_url' ");
	$num=$sql->rowCount();
	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
				...
				$affected_rows=insert('gallery',$posted_data);
				if($affected_rows)
...
if(isset($_POST['show_on_home']))
{
	...
	if(count($arr))
	{
		...
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `gallery` SET show_home='1' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
							<strong></strong>Showing On Home Successfully
						   </div>';	
	}
	else{
...
if(isset($_POST['hide_on_home']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `gallery` SET show_home='0' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
							<strong></strong>Hide from Home Successfully
						   </div>';	
	}
	else{
		$umessage='<div class="alert alert-danger" role="alert">
