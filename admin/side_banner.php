...
if(isset($_POST['addside_banner']))
{
	...
	$pdo=getPDOObject();
	$posted_data=$_POST;
	$sql=$pdo->query("SELECT * FROM `side_banner` where  name LIKE '$name'");
	$num=$sql->rowCount();
	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
				...
				$affected_rows=insert('side_banner',$posted_data);
				if($affected_rows)
...
if(isset($_POST['deleteall']))
{
	...
	$data=sqlfetch("select * from `side_banner` where id in ($str_rest_refs)");
		foreach($data as $side_banner)
		{
			$img_path='../upload/'.$side_banner['photo'];
	...
	$pdo=getPDOObject();
	$q=$pdo->query("DELETE FROM `side_banner` WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['activate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `side_banner` SET actstat='1' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['deactivate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `side_banner` SET actstat='0' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['editdone']))
{
	...
	{	$Filename='';
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);
			...
			$affected_rows=update('side_banner',$posted_data,array('id'=>$pid));
				if($affected_rows)
					$umessage='<div class="alert alert-success" role="alert">
...
function side_banner_form($pid='0',$name='',$name2='',$photo='',$btn_text='',$link='#',$des='',$fld_order='0',$actstat='',$formname='addside_banner')
{ ?>
	<form action="side_banner.php" method="post" enctype="multipart/form-data">
			   ...
			   <label>Line 1</label>
				...
				<input type="text" class="form-control" value="<?php echo $btn_text; ?>" name="btn_text"><br>
				</span>
					
				<span class="col-md-3">
				<label>description</label>
				<textarea type="text" class="form-control" name="des"><?php echo $des; ?></textarea><br>
				</span>
				<span class="col-md-3">
				<label>Link</label>
				<input type="text" class="form-control" value="<?php echo $link; ?>" name="link"><br>
				</span>
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Edit side_banner</h2>
                <div class="box-icon">
            ...
            <div class="box-content row" >
               ...
               <?php 
			$pid=$_GET['pid'];
			$productdata=sqlfetch("SELECT * FROM `side_banner` where id='$pid' ");
			foreach($productdata as $product)
			{
				extract($product);
				side_banner_form($pid,$name,$name2,$photo,$btn_text,$link,$des,$fld_order,$actstat,$formname='editdone');
			} ?>
...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Add side_banner</h2>
                <div class="box-icon">
                    <a href="#" class="btn btn-setting btn-round btn-default"><i
                            class="glyphicon glyphicon-cog"></i></a>
                    <a href="#" class="btn btn-minimize btn-round btn-default"><i
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-edit"></i>side_banner</h2>
            ...
            <div class="box-content">
                <table class="table table-bordered table-striped responsive">
					<tbody>
						...
						$count=1;
						$data=sqlfetch("SELECT * FROM `side_banner`  order by fld_order");
						if(count($data)){
    ...
    if (count($productPages) != 0) {
        ...
        foreach ($productPages as $menu)
						...
						<tr>
							...
							<td><img style="max-width:100px; max-height:100px;" src="../upload/<?php echo $menu['photo']; ?>" class="img-responsive thumbnail" ></td>
							...
							<td>
								...
								<a class="ajax-link" href="side_banner.php?&pid=<?php echo $menu['id']; ?>&edit=true">
								<button type="button" class="btn btn-xs btn-danger pull-right" name="editside_banner">Edit</button>
								</a>
							</td>
						</tr>
						<?php } 
