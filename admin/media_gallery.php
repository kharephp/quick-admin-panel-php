...
check_session();
if(isset($_POST['addmedia_gallery']))
{
	$id=0;
	extract($_POST);
	$type=1;
	$pdo=getPDOObject();
	$posted_data=$_POST;
	$sql=$pdo->query("SELECT * FROM `media_gallery` where  name LIKE '".addcslashes($name,"'")."' or seo_url LIKE '$seo_url' ");
	$num=$sql->rowCount();
	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
				...
				$affected_rows=insert('media_gallery',$posted_data);
				if($affected_rows)
...
if(isset($_POST['deleteall']))
{
	...
	$data=sqlfetch("select * from `media_gallery` where id in ($str_rest_refs)");
		foreach($data as $media_gallery)
		{
			@$img_path='../upload/'.$media_gallery['photo'];
	...
	$pdo=getPDOObject();
	$q=$pdo->query("DELETE FROM `media_gallery` WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['activate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `media_gallery` SET actstat='1' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['deactivate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `media_gallery` SET actstat='0' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['editdone']))
{
	...
	{	$Filename='';
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
			...
			$affected_rows=update('media_gallery',$posted_data,array('id'=>$pid));
				if($affected_rows)
					$umessage='<div class="alert alert-success" role="alert">
...
function media_gallery_form($pid='0',$name='',$type='',$referal_name='',$fld_order='0',$actstat='',$show_in_media=0,$dated='',$seo_url='',$formname='addmedia_gallery')
{ ?>
	<form action="media_gallery.php" method="post" enctype="multipart/form-data">
			...
			<span class="row">
				<span class="col-md-3">
					...
					<input type="text" class="form-control" required name="referal_name" value="<?php echo $referal_name; ?>">
				</span>
				<span class="col-md-3">
				...
				<span class="col-md-3">
				<label>Show on Media Page</label>
					<div class="controls">
						<select name="show_in_media" id="selectError" data-rel="chosen">
							<option <?php if(($show_in_media)=='1')echo 'selected'; ?> value="1">Yes</option>
							<option <?php if(($show_in_media)=='0')echo 'selected'; ?> value="0">No</option>
						</select>
					</div>
				</span>
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Edit media_gallery</h2>
                <div class="box-icon">
            ...
            <div class="box-content row" >
               ...
               <?php 
			$pid=$_GET['pid'];
			$productdata=sqlfetch("SELECT * FROM `media_gallery` where id='$pid' ");
			foreach($productdata as $product)
			{
				extract($product);
				media_gallery_form($pid,$name,$type,$referal_name,$fld_order,$actstat,$show_in_media,$dated,$seo_url,$formname='editdone');
			} ?>
...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Add media_gallery</h2>
                <div class="box-icon">
                    <a href="#" class="btn btn-setting btn-round btn-default"><i
                            class="glyphicon glyphicon-cog"></i></a>
                    <a href="#" class="btn btn-minimize btn-round btn-default"><i
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-edit"></i>media_gallery</h2>
            ...
            <div class="box-content">
                <table class="table table-bordered table-striped responsive">
					<tbody>
						...
						$count=1;
						$data=sqlfetch("SELECT * FROM `media_gallery`  order by fld_order");
						if(count($data)){
        ...
        foreach ($productPages as $menu)
						...
						<tr>
							...
							<td>
								...
								<a class="ajax-link" href="media_gallery.php?&pid=<?php echo $menu['id']; ?>&edit=true">
								<button type="button" class="btn btn-xs btn-danger pull-right" name="editmedia_gallery">Edit</button>
								</a>
							</td>
						</tr>
						<?php }
