	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
				...
				move_uploaded_file($_FILES['photo']['tmp_name'], $target);    //Tells you if its all ok	
				 ...
				 // $q=$pdo->prepare("INSERT into `gallery` values(:name,:photo,:fld_order,:actstat,:seo_url)");
				// $q->execute(array(':name' => $name ,
					...
					$affected_rows =	insert('gallery',$data);	
...
if(isset($_POST['editdone']))
{
	...
	{	$Filename='';
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);
			...
			$affected_rows=update('gallery',$posted_data,array('id'=>$pid));
				if($affected_rows)
					$umessage='<div class="alert alert-success" role="alert">
...
function gallery_form($pid='0',$name='',$photo='',$fld_order='0',$actstat='',$seo_url='',$formname='addgallery')
{ ?>
...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
			...
			{
				extract($product);
				gallery_form($pid,$name,$photo,$fld_order,$actstat,$seo_url,$formname='editdone');
			} ?>
    ...
    if (count($productPages) != 0) {
        ...
        foreach ($productPages as $menu)
						...
						?>
				 		...
				 		<?php echo $pageNumbers = '<div class="pagination">'.$pagination->getLinks($_GET).'</div>';?>
            </div>
        </div>
    </div>
