...
check_session();
if(isset($_POST['addfeatured_media']))
{
	$id=0;
	extract($_POST);
	$type=1;
	$pdo=getPDOObject();
	$posted_data=$_POST;
	// $sql=$pdo->query("SELECT * FROM `featured_media` where  name LIKE '$name' or seo_url LIKE '$seo_url'");
	// $num=$sql->rowCount();
	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
				...
				$affected_rows=insert('featured_media',$posted_data);
				if($affected_rows)
				if($affected_rows)
...
if(isset($_POST['deleteall']))
{
	...
	$data=sqlfetch("select * from `featured_media` where id in ($str_rest_refs)");
		foreach($data as $featured_media)
		{
			$img_path='../upload/'.$featured_media['photo'];
	...
	$pdo=getPDOObject();
	$q=$pdo->query("DELETE FROM `featured_media` WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['activate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `featured_media` SET actstat='1' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['add_slide']))
{
	...
	{
		$str_rest_refs=implode(",",$arr);
		foreach($arr as $featured_media)
		{
			$temp_arr['actstat']=1;
			$temp_arr['featured_media_id']=$featured_media;
			$temp_arr['fld_order']=1;
			
			insert('featured_media',$temp_arr);
...
if(isset($_POST['deactivate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `featured_media` SET actstat='0' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['editdone']))
{
	...
	$affected_rows=update('featured_media',$posted_data,array('id'=>$pid));
				if($affected_rows)
					$umessage='<div class="alert alert-success" role="alert">
...
function featured_media_form($pid='0',$media_id=0,$fld_order='0',$actstat='',$formname='addfeatured_media')
{ ?>
	<form action="featured_media.php" method="post" enctype="multipart/form-data">
			   ...
			   <span class="row">
				...
				<span class="col-md-3">
					<label>Blog</label>
					 <div class="controls">
					<select name="media_id" id="selectError" data-rel="chosen">
					<option>SELECT Media</option>
						<?php
						$categories=sqlfetch("SELECT * FROM `media_item` where actstat='1' and (type='youtube_video' or type='video_browse') order by fld_order");
						foreach($categories as $category)
						{
							$select='';
							if(($media_id==($category['id'])))
								$select='selected';
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Edit featured_media</h2>
                <div class="box-icon">
            ...
            <div class="box-content row" >
               ...
               <?php 
			$pid=$_GET['pid'];
			$productdata=sqlfetch("SELECT * FROM `featured_media` where id='$pid' ");
			foreach($productdata as $product)
			{
				extract($product);
				featured_media_form($pid,$media_id,$fld_order,$actstat,$formname='editdone');
			} ?>
...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Add featured_media</h2>
                <div class="box-icon">
                    <a href="#" class="btn btn-setting btn-round btn-default"><i
                            class="glyphicon glyphicon-cog"></i></a>
                    <a href="#" class="btn btn-minimize btn-round btn-default"><i
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-edit"></i>featured_media</h2>
            ...
            <div class="box-content">
                <table class="table table-bordered table-striped responsive">
					<tbody>
						...
						$count=1;
						$data=sqlfetch("SELECT * FROM `featured_media`  order by fld_order");
						if(count($data)){
    ...
    if (count($productPages) != 0) {
        ...
        foreach ($productPages as $menu)
						...
						<tr>
							...
							<td><a href="media_item.php?pid=<?=$menu['media_id'];?>&edit=true"><?php echo get_media_item_name($menu['media_id']); ?></td>
							...
							<td>
								...
								<a class="ajax-link" href="featured_media.php?&pid=<?php echo $menu['id']; ?>&edit=true">
								<button type="button" class="btn btn-xs btn-danger pull-right" name="editfeatured_media">Edit</button>
								</a>
							</td>
						</tr>
						<?php } 
