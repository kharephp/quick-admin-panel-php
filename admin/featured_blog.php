...
check_session();
if(isset($_POST['addfeatured_blog']))
{
	$id=0;
	extract($_POST);
	$type=1;
	$pdo=getPDOObject();
	$posted_data=$_POST;
	// $sql=$pdo->query("SELECT * FROM `featured_blog` where  name LIKE '$name' or seo_url LIKE '$seo_url'");
	// $num=$sql->rowCount();
	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
				...
				$affected_rows=insert('featured_blog',$posted_data);
				if($affected_rows)
				if($affected_rows)
...
if(isset($_POST['deleteall']))
{
	...
	$data=sqlfetch("select * from `featured_blog` where id in ($str_rest_refs)");
		foreach($data as $featured_blog)
		{
			$img_path='../upload/'.$featured_blog['photo'];
	...
	$pdo=getPDOObject();
	$q=$pdo->query("DELETE FROM `featured_blog` WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['activate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `featured_blog` SET actstat='1' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['add_slide']))
{
	...
	{
		$str_rest_refs=implode(",",$arr);
		foreach($arr as $featured_blog)
		{
			$temp_arr['actstat']=1;
			$temp_arr['featured_blog_id']=$featured_blog;
...
if(isset($_POST['deactivate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `featured_blog` SET actstat='0' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['editdone']))
{
	...
	$affected_rows=update('featured_blog',$posted_data,array('id'=>$pid));
				if($affected_rows)
					$umessage='<div class="alert alert-success" role="alert">
...
function featured_blog_form($pid='0',$blog_id=0,$fld_order='0',$actstat='',$formname='addfeatured_blog')
{ ?>
	<form action="featured_blog.php" method="post" enctype="multipart/form-data">
			   ...
			   <span class="row">
				...
				<span class="col-md-3">
					<label>Blog</label>
					 <div class="controls">
					<select name="blog_id" id="selectError" data-rel="chosen">
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Edit featured_blog</h2>
                <div class="box-icon">
            ...
            <div class="box-content row" >
               ...
               <?php 
			$pid=$_GET['pid'];
			$productdata=sqlfetch("SELECT * FROM `featured_blog` where id='$pid' ");
			foreach($productdata as $product)
			{
				extract($product);
				featured_blog_form($pid,$blog_id,$fld_order,$actstat,$formname='editdone');
			} ?>
...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Add featured_blog</h2>
                <div class="box-icon">
                    <a href="#" class="btn btn-setting btn-round btn-default"><i
                            class="glyphicon glyphicon-cog"></i></a>
                    <a href="#" class="btn btn-minimize btn-round btn-default"><i
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-edit"></i>featured_blog</h2>
            ...
            <div class="box-content">
                <table class="table table-bordered table-striped responsive">
					<tbody>
						...
						$count=1;
						$data=sqlfetch("SELECT * FROM `featured_blog`  order by fld_order");
						if(count($data)){
    ...
    if (count($productPages) != 0) {
        ...
        foreach ($productPages as $menu)
						...
						<tr>
							...
							<td><a href="blog.php?pid=<?=$menu['blog_id'];?>&edit=true"><?php echo get_blog_name($menu['blog_id']); ?></td>
							...
							<td>
								...
								<a class="ajax-link" href="featured_blog.php?&pid=<?php echo $menu['id']; ?>&edit=true">
								<button type="button" class="btn btn-xs btn-danger pull-right" name="editfeatured_blog">Edit</button>
								</a>
							</td>
						</tr>
						<?php } 
