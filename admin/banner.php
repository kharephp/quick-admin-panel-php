...
if(isset($_POST['addbanner']))
{
	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
				...
				$affected_rows=insert('banner',$posted_data);
				if($affected_rows)
...
if(isset($_POST['editdone']))
{
	...
	{	$Filename='';
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);
			...
			$affected_rows=update('banner',$posted_data,array('id'=>$pid));
				if($affected_rows)
					$umessage='<div class="alert alert-success" role="alert">
...
function banner_form($pid='0',$name='',$name2='',$photo='',$btn_text='',$des='#',$fld_order='0',$actstat='',$formname='addbanner')
{ ?>
	<form action="banner.php" method="post" enctype="multipart/form-data">
			   ...
			   <label>Line 1</label>
				...
				<span class="help-text">Image Ratio must be 1366px &times; 640px</span>
...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
			...
			{
				extract($product);
				banner_form($pid,$name,$name2,$photo,$btn_text,$des,$fld_order,$actstat,$formname='editdone');
			} ?>
