...
if(isset($_POST['addconference']))
{
	...
	$pdo=getPDOObject();
	$posted_data=$_POST;
	$sql=$pdo->query("SELECT * FROM `conference` where  name LIKE '$name' or seo_url LIKE '$seo_url' ");
	$num=$sql->rowCount();
	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
				...
				$affected_rows=insert('conference',$posted_data);
				if($affected_rows)
...
if(isset($_POST['deleteall']))
{
	...
	$data=sqlfetch("select * from `conference` where id in ($str_rest_refs)");
		foreach($data as $conference)
		{
			$img_path='../upload/'.$conference['photo'];
	...
	$pdo=getPDOObject();
	$q=$pdo->query("DELETE FROM `conference` WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['activate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `conference` SET actstat='1' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['deactivate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `conference` SET actstat='0' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['editdone']))
{
	...
	{	$Filename='';
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
			...
			$affected_rows=update('conference',$posted_data,array('id'=>$pid));
				if($affected_rows)
					$umessage='<div class="alert alert-success" role="alert">
...
function conference_form($pid='0',$name='',$photo='',$des='',$fld_order='0',$actstat='',$seo_url='',$gallery_id='',$formname='addconference')
{ ?>
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Edit conference</h2>
                <div class="box-icon">
            ...
            <div class="box-content row" >
               ...
               <?php 
			$pid=$_GET['pid'];
			$productdata=sqlfetch("SELECT * FROM `conference` where id='$pid' ");
			foreach($productdata as $product)
			{
				extract($product);
				conference_form($pid,$name,$photo,$des,$fld_order,$actstat,$seo_url,$gallery_id,$formname='editdone');
			} ?>
...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Add conference</h2>
                <div class="box-icon">
                    <a href="#" class="btn btn-setting btn-round btn-default"><i
                            class="glyphicon glyphicon-cog"></i></a>
                    <a href="#" class="btn btn-minimize btn-round btn-default"><i
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-edit"></i>conference</h2>
            ...
            <div class="box-content">
                <table class="table table-bordered table-striped responsive">
					<tbody>
						...
						$count=1;
						$data=sqlfetch("SELECT * FROM `conference`  order by fld_order");
						if(count($data)){
        ...
        foreach ($productPages as $menu)
						...
						<tr>
							...
							<td>
								...
								<a class="ajax-link" href="conference.php?&pid=<?php echo $menu['id']; ?>&edit=true">
								<button type="button" class="btn btn-xs btn-danger pull-right" name="editconference">Edit</button>
								</a>
							</td>
						</tr>
						<?php }
