...
if(isset($_POST['addfront_slide']))
{
	...
	$pdo=getPDOObject();
	$posted_data=$_POST;
	// $sql=$pdo->query("SELECT * FROM `front_slide` where  name LIKE '$name' or seo_url LIKE '$seo_url'");
	// $num=$sql->rowCount();
	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
				...
				$affected_rows=insert('front_slide',$posted_data);
				if($affected_rows)
				if($affected_rows)
...
if(isset($_POST['deleteall']))
{
	...
	$data=sqlfetch("select * from `front_slide` where id in ($str_rest_refs)");
		foreach($data as $front_slide)
		{
			$img_path='../upload/'.$front_slide['photo'];
	...
	$pdo=getPDOObject();
	$q=$pdo->query("DELETE FROM `front_slide` WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['activate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `front_slide` SET actstat='1' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['add_slide']))
{
	...
	{
		$str_rest_refs=implode(",",$arr);
		foreach($arr as $front_slide)
		{
			$temp_arr['actstat']=1;
			$temp_arr['front_slide_id']=$front_slide;
...
if(isset($_POST['deactivate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `front_slide` SET actstat='0' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['editdone']))
{
	...
	$affected_rows=update('front_slide',$posted_data,array('id'=>$pid));
				if($affected_rows)
					$umessage='<div class="alert alert-success" role="alert">
...
function front_slide_form($pid='0',$blog_id=0,$fld_order='0',$actstat='',$formname='addfront_slide')
{ ?>
	<form action="front_slide.php" method="post" enctype="multipart/form-data">
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Edit front_slide</h2>
                <div class="box-icon">
            ...
            <div class="box-content row" >
               ...
               <?php 
			$pid=$_GET['pid'];
			$productdata=sqlfetch("SELECT * FROM `front_slide` where id='$pid' ");
			foreach($productdata as $product)
			{
				extract($product);
				front_slide_form($pid,$blog_id,$fld_order,$actstat,$formname='editdone');
			} ?>
...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Add front_slide</h2>
                <div class="box-icon">
                    <a href="#" class="btn btn-setting btn-round btn-default"><i
                            class="glyphicon glyphicon-cog"></i></a>
                    <a href="#" class="btn btn-minimize btn-round btn-default"><i
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-edit"></i>front_slide</h2>
            ...
            <div class="box-content">
                <table class="table table-bordered table-striped responsive">
					<tbody>
						...
						$count=1;
						$data=sqlfetch("SELECT * FROM `front_slide`  order by fld_order");
						if(count($data)){
        ...
        foreach ($productPages as $menu)
						...
						<tr>
							...
							<td>
								...
								<a class="ajax-link" href="front_slide.php?&pid=<?php echo $menu['id']; ?>&edit=true">
								<button type="button" class="btn btn-xs btn-danger pull-right" name="editfront_slide">Edit</button>
								</a>
							</td>
						</tr>
						<?php }
