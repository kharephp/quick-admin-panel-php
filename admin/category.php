...
if(isset($_POST['addcategory']))
{
	...
	$pdo=getPDOObject();
	$posted_data=$_POST;
	$sql=$pdo->query("SELECT * FROM `category` where  name LIKE '$name' or seo_url LIKE '$seo_url' ");
	$num=$sql->rowCount();
	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
				...
				$affected_rows=insert('category',$posted_data);
				if($affected_rows)
...
if(isset($_POST['editdone']))
{
	...
	{	$Filename='';
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
			...
			$affected_rows=update('category',$posted_data,array('id'=>$pid));
				if($affected_rows)
					$umessage='<div class="alert alert-success" role="alert">
...
function category_form($pid='0',$name='',$photo='',$des='',$fld_order='0',$actstat='',$icon='',$color='',$seo_url='',$formname='addcategory')
{ ?>
...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
			...
			{
				extract($product);
				category_form($pid,$name,$photo,$des,$fld_order,$actstat,$icon,$color,$seo_url,$formname='editdone');
			} ?>
